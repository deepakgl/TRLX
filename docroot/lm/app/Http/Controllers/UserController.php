...
class UserController extends Controller {
  ...
  private function updateUserIndex($data) {
    ...
    if ($exist && !empty($uid)) {
      ...
      $data['uid'] = $uid;
      $params['body']['doc'] = $this->createUserBody($elastic_arr, $data, 'update');
  ...
  private function createUserBody($elastic_arr, $data, $action = 'add') {
    ...
    $elastic_arr['status'] = $data['status'];
  ...
  public function getUsersListing(Request $request) {
    ...
    if (!empty($request->input('uid'))) {
      $queryParams = ElasticUserModel::getElasticSearchParam('uid', $request->input('uid'), $this->limit, $this->offset);
    }
    else {
