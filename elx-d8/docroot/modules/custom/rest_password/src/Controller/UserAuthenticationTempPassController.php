 ...
 *
 * This is copy paste of user authentication controller with one small change.
 */
class UserAuthenticationTempPassController extends ControllerBase implements ContainerInjectionInterface {
  ...
  /**
   * Constructs a new user authentication controller object.
   *
  ...
  protected function floodControl(Request $request, $username) {
    ...
    if (!$this->flood->isAllowed('user.failed_login_ip', $flood_config->get('ip_limit'), $flood_config->get('ip_window'))) {
      throw new AccessDeniedHttpException(
        'Access is blocked because of IP based flood prevention.', NULL, Response::HTTP_TOO_MANY_REQUESTS
      );
    }
      ...
      if (!$this->flood->isAllowed('user.http_login', $flood_config->get('user_limit'), $flood_config->get('user_window'), $identifier)) {
        if ($flood_config->get('uid_only')) {
          $error_message = sprintf(
            'There have been more than %s failed login attempts for this
            account. It is temporarily blocked. Try again later or request
            a new password.', $flood_config->get('user_limit')
          );
        }
        else {
